                      ...
                      size_t *upload_data_size, void **con_cls)
{
  ...
  for (i=0; i < n; i++) {
    ...
    else if (strcmp(dict[i].key, "t_out") == 0)
      t_out = atoi(dict[i].value);
  ...
  */
  ...
  Greenhouse__greenhouse_step(crop, t_in, t_out, time, &_res, &mem);

  strcat(json,"{");
  strcat(json,"\"model\":\"temperature_based_v3_t_out\"");
